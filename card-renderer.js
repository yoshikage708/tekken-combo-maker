...
function canvasNumpad(c,x,y,size,label,held=false){c.fillStyle=held?'#f1b84b':'#f5f7fa';const scale=label==='☆'?0.76:1.08;c.font=`800 ${Math.round(size*scale)}px Arial,"Yu Gothic",sans-serif`;c.textAlign='center';c.textBaseline='middle';c.fillText(label,x+size/2,y+size/2+2);c.textAlign='left';c.textBaseline='alphabetic'}
...
function drawComboIcons(c,startX,startY,maxX){
  ...
  combo.forEach((item,index)=>{
    ...
    if(item.type==='direction'){
      if(directionDisplay==='numpad')canvasNumpad(c,tx-7,ty+1,36,item.label,item.hold)
