...
function canvasNumpad(c,x,y,size,label,filled=false){c.beginPath();c.roundRect(x,y,size,size,7);if(filled){c.fillStyle='#f5f7fa';c.fill()}else{c.strokeStyle='#f5f7fa';c.lineWidth=2;c.stroke()}c.fillStyle=filled?'#171d24':'#f5f7fa';c.font=`900 ${Math.round(size*.58)}px Inter,"Yu Gothic",sans-serif`;c.textAlign='center';c.textBaseline='middle';c.fillText(label,x+size/2,y+size/2+1);c.textAlign='left';c.textBaseline='alphabetic'}
...
function drawComboIcons(c,startX,startY,maxX){
  ...
  combo.forEach(item=>{
    ...
    if(item.type==='direction'){
      if(directionDisplay==='numpad')canvasNumpad(c,tx,ty+4,42,item.label==='☆'?'5':item.label,item.hold)
